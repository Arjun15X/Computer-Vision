...
while (cap.isOpened()):
    ...
    if ret == True:
        ...
        faces = cv2.CascadeClassifier('C:\\opencv\\build\\etc\\haarcascades\\haarcascade_eye.xml')
        detected = faces.detectMultiScale(frame, 1.3, 5)

        # faces = cv2.CascadeClassifier('haarcascade_frontalface_default.xml')
        # detected2 = faces.detectMultiScale(frameDBW, 1.3, 5)
        # windowErode = np.ones ((2,5), np.uint8)
        pupilFrame = frame
        ...
        for (x, y, w, h) in detected:
            ...
            if len(contours) >= 2:
                ...
                for cnt in contours:
                    ...
                    cx, cy = int(center['m10'] / 6), int(center['m01'] / 8)
                    distanceX.append(cx)
                    if area > maxArea:
                        maxArea = area
